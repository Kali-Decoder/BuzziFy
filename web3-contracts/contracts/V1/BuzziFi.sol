...
contract BuzziFi {
    ...
    function _calculationReward(uint256 _pool_id) private {
        ...
        for (uint i = 0; i < userBets.length; i++) {
            ...
            uint256 _accuracyScore = 1 / (divergence + 1);
        ...
        for (uint i = 0; i < userBets.length; i++) {
            ...
            uint256 _accuracyScore = 1 / (divergence + 1);
            uint256 weightedAccuracy = _accuracyScore * userBets[i].amount;
            // uint256 deduction = pool.total_amount - pool.total_amount * FEE;
           uint256 _reward = (weightedAccuracy / _totalWeightedAccuracy) * pool.total_amount;
            if (_reward > 0) {
    ...
    function claimBet(uint _pool_id) external {
        Pool storage pool = pools[_pool_id];
