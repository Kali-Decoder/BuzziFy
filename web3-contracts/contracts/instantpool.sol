...
contract InstantPool {
    ...
    function instantlyDistribute() external payable  {
        uint256 _amount = msg.value;
        uint256 _amountDistribute = (_amount*95)/100;
        acceptedToken.upgradeByETH{value:_amountDistribute}();
        acceptedToken.distributeToPool(address(this),pool,_amountDistribute);
    }

    function giveUnits(address[] memory _members,uint256[] memory _giveUnits) external onlyOwner {
        for(uint256 i=0;i<_members.length;i++){
            acceptedToken.updateMemberUnits(pool,_members[i],_giveUnits[i]);
        }
    }

    function deleteMemberFromPool(address[] memory _members) external onlyOwner {
        for(uint256 i=0;i<_members.length;i++){
            acceptedToken.updateMemberUnits(pool,_members[i],0);
        }
    }
